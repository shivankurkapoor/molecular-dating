...
'''
def download_at_intervals():
    request_gen = engine.scan(TaxaRequest).filter(are_files_downloaded=False).gen()
    request = next(request_gen)
    print engine.scan(TaxaRequest).filter(are_files_downloaded=False).count()
    request_dict = request.__dict__
    print request_dict['userId']
    print request_dict['are_files_downloaded']
    if request:
        request_dict = request.__dict__
        user_id = request_dict['userId']
        request_id = request_dict['requestId']
        files_reqd = str(request_dict['file_json'])
        print 'user_id: ', user_id
        print 'request_id: ', request_id
        print 'files_reqd: ', files_reqd
        status = download_files(user_id, request_id, files_reqd)
        if status == INT_DOWNLOADED:
            request.set_are_files_downloaded(True)
            try:
                request.sync()
            except Exception as e:
                print 'Error in saving request details in database ',e
    else:
        print 'all files downloaded'

download_at_intervals()
'''

def process_first_stage_at_intervals():
    output_dir = sys.argv[1].replace('\'', '')
    forward_fastq_file = sys.argv[2].replace('\'', '')
    backward_fastq_file = sys.argv[3].replace('\'', '')
    fps = sys.argv[4]
    bps = sys.argv[5]
    seq_len = int(sys.argv[6])
    percent = float(sys.argv[7])
    base_count = int(sys.argv[8])
    output_file_name = output_dir.split('/')[-1] + '.fasta'
    output_file_name = output_dir + '/' + output_file_name
    process_pipeline(fps, bps, seq_len, percent, base_count, output_file_name, forward_fastq_file, backward_fastq_file)

process_first_stage_at_intervals()
